...
if [ "$SETUP_LETSENCRYPT" = true ]; then
    ...
    curl -o /etc/nginx/sites-available/phpmyadmin.conf "https://raw.githubusercontent.com/BAERSERK/pterodactyl-script/master/configs/phpmyadmin.conf"
...
if [ "$NGINX_SSL" = true ]; then
    curl -o /etc/nginx/sites-available/phpmyadmin.conf "https://raw.githubusercontent.com/BAERSERK/pterodactyl-script/master/configs/phpmyadmin_ssl.conf"
